    ...
    <main className="min-h-screen bg-[#1f2121] text-[#f5f5f5] pb-24">
      <div className="max-w-2xl mx-auto p-4 md:p-8">
        ...
        {crewId ? (
          <>
            {/* Crew info */}
            <GlassCard className="mb-4 border-t-2 border-[#32b8c6]">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h3 className="font-black text-xl text-white glow-teal tracking-wide">Crew Active</h3>
                  <p className="text-xs text-[var(--color-text-secondary)] font-mono mt-1">SECURE_LINK: <span className="text-[#32b8c6]">{crewId}</span></p>
                </div>
                <button
                  onClick={() => {
                    navigator.clipboard?.writeText(crewId);
                  }}
                  className="magnetic-btn text-xs px-4 py-2 rounded-full border border-[#32b8c6]/30 bg-[#32b8c6]/10 text-[#32b8c6] hover:bg-[#32b8c6]/20 transition-all shadow-[0_0_10px_rgba(50,184,198,0.2)]"
                >
                  📋 COPY CIPHER
                </button>
              </div>
              ...
              <div className="space-y-3">
                {members.map((member, i) => (
                  <motion.div
                    ...
                    className="flex items-center gap-4 p-3 rounded-xl bg-[#1f2121]/50 border border-white/5 hover:bg-white/10 transition-colors"
                  >
          ...
          <div className="space-y-6">
            <GlassCard className="text-center py-10 relative overflow-hidden group border-t border-[#32b8c6]/30">
              <div className="absolute inset-0 mesh-gradient opacity-50 group-hover:opacity-100 transition-opacity duration-1000" />
              <div className="relative z-10 w-24 h-24 mx-auto mb-6 bg-gradient-to-br from-[#32b8c6]/20 to-purple-500/20 rounded-full flex items-center justify-center">
                <span className="text-5xl group-hover:scale-125 transition-transform duration-500 delay-100">🤝</span>
                <div className="absolute inset-0 rounded-full border border-[#32b8c6]/50 animate-ping opacity-20" style={{ animationDuration: '3s' }} />
              </div>
              ...
              <div className="flex flex-col gap-4 max-w-sm mx-auto">
                <button
                  ...
                  className="magnetic-btn py-4 bg-gradient-to-r from-[#21808d] to-purple-600 text-white rounded-xl font-bold shadow-[0_0_15px_rgba(50,184,198,0.4)] transition-all active:scale-95"
                >
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-[2] bg-[#1a1c1c] border border-white/10 rounded-xl px-4 py-3.5 text-sm text-center text-white placeholder-[var(--color-text-secondary)] font-mono uppercase tracking-widest focus:outline-none focus:border-[#32b8c6] focus:ring-1 focus:ring-[#32b8c6]/50 transition-all shadow-inner"
                    maxLength={6}
                  />
                  <button
                    onClick={() => shareCode && createCrew(shareCode)}
                    disabled={shareCode.length < 3}
                    className="flex-1 magnetic-btn px-4 py-3.5 bg-white/5 border border-[#32b8c6]/50 text-[#32b8c6] rounded-xl hover:bg-[#32b8c6]/20 disabled:opacity-50 transition-all font-bold tracking-wide"
                  >
                    SYNC
                  </button>
                </div>
